...
const (
	testAccAbrhaSpacesBucketCorsConfiguration_TestRegion = "nyc3"
)

func TestAccAbrhaSpacesBucketCorsConfiguration_basic(t *testing.T) {
	name := acceptance.RandomTestName()
	ctx := context.Background()
	region := testAccAbrhaSpacesBucketCorsConfiguration_TestRegion
	resourceName := "abrha_spaces_bucket_cors_configuration.test"

	resource.ParallelTest(t, resource.TestCase{
		PreCheck:          func() { acceptance.TestAccPreCheck(t) },
		ProviderFactories: acceptance.TestAccProviderFactories,
		CheckDestroy:      testAccCheckAbrhaSpacesBucketCorsConfigurationDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccSpacesBucketCORSConfigurationConfig_basic(name, region, "https://www.example.com"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAbrhaSpacesBucketCorsConfigurationExists(ctx, resourceName),
...
func TestAccAbrhaSpacesBucketCorsConfiguration_SingleRule(t *testing.T) {
	resourceName := "abrha_spaces_bucket_cors_configuration.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAbrhaSpacesBucketCorsConfigurationExists(ctx, resourceName),
					resource.TestCheckResourceAttrPair(resourceName, "bucket", "abrha_spaces_bucket.foobar", "id"),
					resource.TestCheckResourceAttr(resourceName, "cors_rule.#", "1"),
					resource.TestCheckTypeSetElemNestedAttrs(resourceName, "cors_rule.*", map[string]string{
						"allowed_headers.#": "1",
						"allowed_methods.#": "3",
...
func TestAccAbrhaSpacesBucketCorsConfiguration_MultipleRules(t *testing.T) {
	resourceName := "abrha_spaces_bucket_cors_configuration.test"
	rName := acceptance.RandomTestName()
	ctx := context.Background()
	region := testAccAbrhaSpacesBucketCorsConfiguration_TestRegion
...
func TestAccAbrhaSpacesBucketCorsConfiguration_update(t *testing.T) {
	resourceName := "abrha_spaces_bucket_cors_configuration.test"
	rName := acceptance.RandomTestName()
	ctx := context.Background()
	region := testAccAbrhaSpacesBucketCorsConfiguration_TestRegion
...
func testAccGetS3CorsConfigurationConn() (*s3.S3, error) {
	client, err := acceptance.TestAccProvider.Meta().(*config.CombinedConfig).SpacesClient(testAccAbrhaSpacesBucketCorsConfiguration_TestRegion)
	if err != nil {
		return nil, err
	}
...
func testAccCheckAbrhaSpacesBucketCorsConfigurationExists(ctx context.Context, resourceName string) resource.TestCheckFunc {
...
func testAccCheckAbrhaSpacesBucketCorsConfigurationDestroy(s *terraform.State) error {
	s3conn, err := testAccGetS3CorsConfigurationConn()
	if err != nil {
		return err
	}
	...
	for _, rs := range s.RootModule().Resources {
		switch rs.Type {
		case "abrha_spaces_bucket_cors_configuration":
...
resource "abrha_spaces_bucket_cors_configuration" "test" {
  bucket = abrha_spaces_bucket.foobar.id
  region = "nyc3"
