...
func TestAccAbrhaTag_NameValidation(t *testing.T) {
	cases := []struct {
		Input       string
		ExpectError bool
	}{
	...
	for _, tc := range cases {
		...
		if tc.ExpectError && !hasError {
			t.Fatalf("Expected the Abrha Tag Name to trigger a validation error for '%s'", tc.Input)
		}

		if hasError && !tc.ExpectError {
			t.Fatalf("Unexpected error validating the Abrha Tag Name '%s': %s", tc.Input, errors[0])
		}
	}
}
