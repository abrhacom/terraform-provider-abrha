...
func loadbalancerStateRefreshFunc(client *goApiAbrha.Client, loadbalancerId string) retry.StateRefreshFunc {
	return func() (interface{}, string, error) {
		lb, _, err := client.LoadBalancers.Get(context.Background(), loadbalancerId)
		if err != nil {
			return nil, "", fmt.Errorf("Error issuing read request in LoadbalancerStateRefreshFunc to Abrha for Load Balancer '%s': %s", loadbalancerId, err)
		}
...
func expandStickySessions(config []interface{}) *goApiAbrha.StickySessions {
	stickysessionConfig := config[0].(map[string]interface{})

	stickySession := &goApiAbrha.StickySessions{
...
func expandLBFirewall(config []interface{}) *goApiAbrha.LBFirewall {
	firewallConfig := config[0].(map[string]interface{})

	firewall := &goApiAbrha.LBFirewall{}
...
func expandHealthCheck(config []interface{}) *goApiAbrha.HealthCheck {
	healthcheckConfig := config[0].(map[string]interface{})

	healthcheck := &goApiAbrha.HealthCheck{
...
func expandForwardingRules(client *goApiAbrha.Client, config []interface{}) ([]goApiAbrha.ForwardingRule, error) {
	forwardingRules := make([]goApiAbrha.ForwardingRule, 0, len(config))

	for _, rawRule := range config {
		rule := rawRule.(map[string]interface{})

		r := goApiAbrha.ForwardingRule{
...
func flattenVmIds(list []string) *schema.Set {
...
func flattenHealthChecks(health *goApiAbrha.HealthCheck) []map[string]interface{} {
...
func flattenStickySessions(session *goApiAbrha.StickySessions) []map[string]interface{} {
	result := make([]map[string]interface{}, 0, 1)
...
func flattenLBFirewall(firewall *goApiAbrha.LBFirewall) []map[string]interface{} {
	result := make([]map[string]interface{}, 0, 1)
...
func flattenForwardingRules(client *goApiAbrha.Client, rules []goApiAbrha.ForwardingRule) ([]map[string]interface{}, error) {
	result := make([]map[string]interface{}, 0, 1)
...
func expandDomains(client *goApiAbrha.Client, config []interface{}) ([]*goApiAbrha.LBDomain, error) {
	domains := make([]*goApiAbrha.LBDomain, 0, len(config))

	for _, rawDomain := range config {
		domain := rawDomain.(map[string]interface{})
		r := &goApiAbrha.LBDomain{Name: domain["name"].(string)}
...
func expandGLBSettings(config []interface{}) *goApiAbrha.GLBSettings {
	glbConfig := config[0].(map[string]interface{})

	glbSettings := &goApiAbrha.GLBSettings{
	...
	if v, ok := glbConfig["cdn"]; ok {
		if raw := v.([]interface{}); len(raw) > 0 {
			glbSettings.CDN = &goApiAbrha.CDNSettings{
...
func flattenDomains(client *goApiAbrha.Client, domains []*goApiAbrha.LBDomain) ([]map[string]interface{}, error) {
	if len(domains) == 0 {
		return nil, nil
	}
...
func flattenGLBSettings(settings *goApiAbrha.GLBSettings) []map[string]interface{} {
	result := make([]map[string]interface{}, 0, 1)
