...
func expandNodePools(nodePools []interface{}) []*goApiAbrha.KubernetesNodePool {
	expandedNodePools := make([]*goApiAbrha.KubernetesNodePool, 0, len(nodePools))
	for _, rawPool := range nodePools {
		pool := rawPool.(map[string]interface{})
		cr := &goApiAbrha.KubernetesNodePool{
			ID:        pool["id"].(string),
...
func expandMaintPolicyOpts(config []interface{}) (*goApiAbrha.KubernetesMaintenancePolicy, error) {
	maintPolicy := &goApiAbrha.KubernetesMaintenancePolicy{}
	configMap := config[0].(map[string]interface{})

	if v, ok := configMap["day"]; ok {
		day, err := goApiAbrha.KubernetesMaintenanceToDay(v.(string))
		if err != nil {
			return nil, err
		}
		maintPolicy.Day = day
...
func flattenMaintPolicyOpts(opts *goApiAbrha.KubernetesMaintenancePolicy) []map[string]interface{} {
...
func flattenNodePool(d *schema.ResourceData, keyPrefix string, pool *goApiAbrha.KubernetesNodePool, parentTags ...string) []interface{} {
...
func expandNodes(nodes []interface{}) []*goApiAbrha.KubernetesNode {
	expandedNodes := make([]*goApiAbrha.KubernetesNode, 0, len(nodes))
	for _, rawNode := range nodes {
		node := rawNode.(map[string]interface{})
		n := &goApiAbrha.KubernetesNode{
			ID:   node["id"].(string),
...
func flattenNodes(nodes []*goApiAbrha.KubernetesNode) []interface{} {
...
func expandNodePoolTaints(taints []interface{}) []goApiAbrha.Taint {
	expandedTaints := make([]goApiAbrha.Taint, 0, len(taints))
...
func flattenNodePoolTaints(taints []goApiAbrha.Taint) []interface{} {
