...
func appSpecSchema(isResource bool) map[string]*schema.Schema {
	spec := map[string]*schema.Schema{
		...
		"region": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "The slug for the Abrha data center region hosting the app",
		},
		"domain": {
	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			...
			"zone": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "If the domain uses Abrha DNS and you would like App Platform to automatically manage it for you, set this to the name of the domain on your account.",
			},
		},
	}
}
	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"rule": {
				...
				Required: true,
				ValidateFunc: validation.StringInSlice([]string{
					string(goApiAbrha.AppAlertSpecRule_DeploymentFailed),
					string(goApiAbrha.AppAlertSpecRule_DeploymentLive),
					string(goApiAbrha.AppAlertSpecRule_DeploymentStarted),
					string(goApiAbrha.AppAlertSpecRule_DeploymentCanceled),
					string(goApiAbrha.AppAlertSpecRule_DomainFailed),
					string(goApiAbrha.AppAlertSpecRule_DomainLive),
				}, false),
			},
			"disabled": {
				Type:     schema.TypeBool,
	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			"rule": {
				...
				Required: true,
				ValidateFunc: validation.StringInSlice([]string{
					string(goApiAbrha.AppAlertSpecRule_UnspecifiedRule),
					string(goApiAbrha.AppAlertSpecRule_CPUUtilization),
					string(goApiAbrha.AppAlertSpecRule_MemUtilization),
					string(goApiAbrha.AppAlertSpecRule_RestartCount),
				}, false),
			},
			"operator": {
				Type:     schema.TypeString,
				Required: true,
				ValidateFunc: validation.StringInSlice([]string{
					string(goApiAbrha.AppAlertSpecOperator_GreaterThan),
					string(goApiAbrha.AppAlertSpecOperator_LessThan),
					string(goApiAbrha.AppAlertSpecOperator_UnspecifiedOperator),
				}, false),
			},
			"window": {
				Type:     schema.TypeString,
				Required: true,
				ValidateFunc: validation.StringInSlice([]string{
					string(goApiAbrha.AppAlertSpecWindow_FiveMinutes),
					string(goApiAbrha.AppAlertSpecWindow_TenMinutes),
					string(goApiAbrha.AppAlertSpecWindow_ThirtyMinutes),
					string(goApiAbrha.AppAlertSpecWindow_OneHour),
					string(goApiAbrha.AppAlertSpecWindow_UnspecifiedWindow),
				}, false),
			},
			"value": {
	...
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			...
			"cluster_name": {
				Type:        schema.TypeString,
				Optional:    true,
				Description: "The name of the underlying Abrha DBaaS cluster. This is required for production databases. For dev databases, if cluster_name is not set, a new cluster will be provisioned.",
			},
			"db_name": {
				Type:        schema.TypeString,
				Optional:    true,
...
func expandAppSpec(config []interface{}) *goApiAbrha.AppSpec {
	if len(config) == 0 || config[0] == nil {
		return &goApiAbrha.AppSpec{}
	}
	appSpecConfig := config[0].(map[string]interface{})

	appSpec := &goApiAbrha.AppSpec{
...
func flattenAppSpec(d *schema.ResourceData, spec *goApiAbrha.AppSpec) []map[string]interface{} {
...
func expandAppAlerts(config []interface{}) []*goApiAbrha.AppAlertSpec {
	appAlerts := make([]*goApiAbrha.AppAlertSpec, 0, len(config))

	for _, rawAlert := range config {
		alert := rawAlert.(map[string]interface{})

		a := &goApiAbrha.AppAlertSpec{
			Rule:     goApiAbrha.AppAlertSpecRule(alert["rule"].(string)),
			Disabled: alert["disabled"].(bool),
		}

		if alert["operator"] != nil {
			a.Operator = goApiAbrha.AppAlertSpecOperator(alert["operator"].(string))
		}
		if alert["window"] != nil {
			a.Window = goApiAbrha.AppAlertSpecWindow(alert["window"].(string))
		}
		if alert["value"] != nil {
...
func flattenAppAlerts(alerts []*goApiAbrha.AppAlertSpec) []map[string]interface{} {
...
func expandAppLogDestinations(config []interface{}) []*goApiAbrha.AppLogDestinationSpec {
	logDestinations := make([]*goApiAbrha.AppLogDestinationSpec, 0, len(config))

	for _, rawDestination := range config {
		destination := rawDestination.(map[string]interface{})

		d := &goApiAbrha.AppLogDestinationSpec{
		...
		if len(open_search) > 0 {
			...
			d.OpenSearch = &goApiAbrha.AppLogDestinationSpecOpenSearch{
		...
		if len(papertrail) > 0 {
			...
			d.Papertrail = &goApiAbrha.AppLogDestinationSpecPapertrail{
		...
		if len(datadog) > 0 {
			...
			d.Datadog = &goApiAbrha.AppLogDestinationSpecDataDog{
		...
		if len(logtail) > 0 {
			...
			d.Logtail = &goApiAbrha.AppLogDestinationSpecLogtail{
...
func flattenAppLogDestinations(destinations []*goApiAbrha.AppLogDestinationSpec) []map[string]interface{} {
...
func expandAppOpensearchBasicAuth(config []interface{}) *goApiAbrha.OpenSearchBasicAuth {
	basicAuthConfig := config[0].(map[string]interface{})

	basicAuth := &goApiAbrha.OpenSearchBasicAuth{
...
func expandAppAutoscaling(config []interface{}) *goApiAbrha.AppAutoscalingSpec {
	autoscalingConfig := config[0].(map[string]interface{})

	autoscalingSpec := &goApiAbrha.AppAutoscalingSpec{
...
func expandAppAutoscalingMetrics(config []interface{}) *goApiAbrha.AppAutoscalingSpecMetrics {
	metrics := &goApiAbrha.AppAutoscalingSpecMetrics{}
	...
	for _, rawMetric := range config {
		...
		if len(cpu) > 0 {
			...
			metrics.CPU = &goApiAbrha.AppAutoscalingSpecMetricCPU{
...
func flattenAppAutoscaling(autoscaling *goApiAbrha.AppAutoscalingSpec) []map[string]interface{} {
...
func expandAppDomainSpec(config []interface{}) []*goApiAbrha.AppDomainSpec {
	appDomains := make([]*goApiAbrha.AppDomainSpec, 0, len(config))

	for _, rawDomain := range config {
		domain := &goApiAbrha.AppDomainSpec{
...
func expandAppSpecDomains(config []interface{}) []*goApiAbrha.AppDomainSpec {
	appDomains := make([]*goApiAbrha.AppDomainSpec, 0, len(config))

	for _, rawDomain := range config {
		domain := rawDomain.(map[string]interface{})

		d := &goApiAbrha.AppDomainSpec{
			Domain:   domain["name"].(string),
			Type:     goApiAbrha.AppDomainSpecType(domain["type"].(string)),
...
func flattenAppDomainSpec(spec []*goApiAbrha.AppDomainSpec) *schema.Set {
...
func flattenAppSpecDomains(domains []*goApiAbrha.AppDomainSpec) []map[string]interface{} {
...
func expandAppGitHubSourceSpec(config []interface{}) *goApiAbrha.GitHubSourceSpec {
	gitHubSourceConfig := config[0].(map[string]interface{})

	gitHubSource := &goApiAbrha.GitHubSourceSpec{
...
func flattenAppGitHubSourceSpec(spec *goApiAbrha.GitHubSourceSpec) []interface{} {
...
func expandAppGitLabSourceSpec(config []interface{}) *goApiAbrha.GitLabSourceSpec {
	gitLabSourceConfig := config[0].(map[string]interface{})

	gitLabSource := &goApiAbrha.GitLabSourceSpec{
...
func flattenAppGitLabSourceSpec(spec *goApiAbrha.GitLabSourceSpec) []interface{} {
	result := make([]interface{}, 0)
...
func expandAppGitSourceSpec(config []interface{}) *goApiAbrha.GitSourceSpec {
	gitSourceConfig := config[0].(map[string]interface{})

	gitSource := &goApiAbrha.GitSourceSpec{
...
func flattenAppGitSourceSpec(spec *goApiAbrha.GitSourceSpec) []interface{} {
	result := make([]interface{}, 0)
...
func expandAppImageSourceSpec(config []interface{}) *goApiAbrha.ImageSourceSpec {
	imageSourceConfig := config[0].(map[string]interface{})

	imageSource := &goApiAbrha.ImageSourceSpec{
		RegistryType:        goApiAbrha.ImageSourceSpecRegistryType(imageSourceConfig["registry_type"].(string)),
	...
	if len(docrPush) > 0 {
		...
		imageSource.DeployOnPush = &goApiAbrha.ImageSourceSpecDeployOnPush{
...
func flattenAppImageSourceSpec(i *goApiAbrha.ImageSourceSpec) []interface{} {
	result := make([]interface{}, 0)
...
func expandAppEnvs(config []interface{}) []*goApiAbrha.AppVariableDefinition {
	appEnvs := make([]*goApiAbrha.AppVariableDefinition, 0, len(config))

	for _, rawEnv := range config {
		env := rawEnv.(map[string]interface{})

		e := &goApiAbrha.AppVariableDefinition{
			Value: env["value"].(string),
			Scope: goApiAbrha.AppVariableScope(env["scope"].(string)),
			Key:   env["key"].(string),
			Type:  goApiAbrha.AppVariableType(env["type"].(string)),
		}
...
func flattenAppEnvs(appEnvs []*goApiAbrha.AppVariableDefinition) *schema.Set {
...
func expandAppHealthCheck(config []interface{}) *goApiAbrha.AppServiceSpecHealthCheck {
	healthCheckConfig := config[0].(map[string]interface{})

	healthCheck := &goApiAbrha.AppServiceSpecHealthCheck{
...
func flattenAppHealthCheck(check *goApiAbrha.AppServiceSpecHealthCheck) []interface{} {
	result := make([]interface{}, 0)
...
func expandAppRoutes(config []interface{}) []*goApiAbrha.AppRouteSpec {
	appRoutes := make([]*goApiAbrha.AppRouteSpec, 0, len(config))

	for _, rawRoute := range config {
		route := rawRoute.(map[string]interface{})

		r := &goApiAbrha.AppRouteSpec{
...
func flattenAppRoutes(routes []*goApiAbrha.AppRouteSpec) []interface{} {
	result := make([]interface{}, 0)
...
func expandAppSpecServices(config []interface{}) []*goApiAbrha.AppServiceSpec {
	appServices := make([]*goApiAbrha.AppServiceSpec, 0, len(config))

	for _, rawService := range config {
		service := rawService.(map[string]interface{})

		s := &goApiAbrha.AppServiceSpec{
...
func flattenAppSpecServices(services []*goApiAbrha.AppServiceSpec) []map[string]interface{} {
...
func expandAppSpecStaticSites(config []interface{}) []*goApiAbrha.AppStaticSiteSpec {
	appSites := make([]*goApiAbrha.AppStaticSiteSpec, 0, len(config))

	for _, rawSite := range config {
		site := rawSite.(map[string]interface{})

		s := &goApiAbrha.AppStaticSiteSpec{
...
func flattenAppSpecStaticSites(sites []*goApiAbrha.AppStaticSiteSpec) []map[string]interface{} {
...
func expandAppSpecWorkers(config []interface{}) []*goApiAbrha.AppWorkerSpec {
	appWorkers := make([]*goApiAbrha.AppWorkerSpec, 0, len(config))

	for _, rawWorker := range config {
		worker := rawWorker.(map[string]interface{})

		s := &goApiAbrha.AppWorkerSpec{
...
func flattenAppSpecWorkers(workers []*goApiAbrha.AppWorkerSpec) []map[string]interface{} {
...
func expandAppSpecJobs(config []interface{}) []*goApiAbrha.AppJobSpec {
	appJobs := make([]*goApiAbrha.AppJobSpec, 0, len(config))

	for _, rawJob := range config {
		job := rawJob.(map[string]interface{})

		s := &goApiAbrha.AppJobSpec{
			...
			Kind:             goApiAbrha.AppJobSpecKind(job["kind"].(string)),
		}
...
func flattenAppSpecJobs(jobs []*goApiAbrha.AppJobSpec) []map[string]interface{} {
...
func expandAppSpecFunctions(config []interface{}) []*goApiAbrha.AppFunctionsSpec {
	appFn := make([]*goApiAbrha.AppFunctionsSpec, 0, len(config))

	for _, rawFn := range config {
		fn := rawFn.(map[string]interface{})

		f := &goApiAbrha.AppFunctionsSpec{
			Name:      fn["name"].(string),
...
func flattenAppSpecFunctions(functions []*goApiAbrha.AppFunctionsSpec) []map[string]interface{} {
...
func expandAppSpecDatabases(config []interface{}) []*goApiAbrha.AppDatabaseSpec {
	appDatabases := make([]*goApiAbrha.AppDatabaseSpec, 0, len(config))

	for _, rawDatabase := range config {
		db := rawDatabase.(map[string]interface{})

		s := &goApiAbrha.AppDatabaseSpec{
			Name:        db["name"].(string),
			Engine:      goApiAbrha.AppDatabaseSpecEngine(db["engine"].(string)),
...
func flattenAppSpecDatabases(databases []*goApiAbrha.AppDatabaseSpec) []map[string]interface{} {
...
func expandAppCORSPolicy(config []interface{}) *goApiAbrha.AppCORSPolicy {
	if len(config) == 0 || config[0] == nil {
		return nil
	}
	...
	var allowOrigins []*goApiAbrha.AppStringMatch
	if len(allowOriginsConfig) > 0 {
		allowOriginsMap := allowOriginsConfig[0].(map[string]interface{})

		if allowOriginsMap["exact"] != "" {
			allowOrigins = append(allowOrigins, &goApiAbrha.AppStringMatch{Exact: allowOriginsMap["exact"].(string)})
		}
		if allowOriginsMap["prefix"] != "" {
			allowOrigins = append(allowOrigins, &goApiAbrha.AppStringMatch{Prefix: allowOriginsMap["prefix"].(string)})
		}
		if allowOriginsMap["regex"] != "" {
			allowOrigins = append(allowOrigins, &goApiAbrha.AppStringMatch{Regex: allowOriginsMap["regex"].(string)})
		}
	}
	...
	return &goApiAbrha.AppCORSPolicy{
...
func flattenAppCORSPolicy(policy *goApiAbrha.AppCORSPolicy) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)
...
func expandAppIngress(config []interface{}) *goApiAbrha.AppIngressSpec {
	if len(config) == 0 || config[0] == nil {
		return nil
	}

	ingress := &goApiAbrha.AppIngressSpec{}
	ingressConfig := config[0].(map[string]interface{})
	rules := ingressConfig["rule"].([]interface{})

	for _, r := range rules {
		rule := r.(map[string]interface{})
		result := &goApiAbrha.AppIngressSpecRule{
...
func expandAppEgress(config []interface{}) *goApiAbrha.AppEgressSpec {
	if len(config) == 0 || config[0] == nil {
		return nil
	}
	egressConfig := config[0].(map[string]interface{})

	return &goApiAbrha.AppEgressSpec{
		Type: goApiAbrha.AppEgressSpecType(egressConfig["type"].(string)),
	}
}

func expandAppIngressComponent(config []interface{}) *goApiAbrha.AppIngressSpecRuleRoutingComponent {
	if len(config) == 0 || config[0] == nil {
		return nil
	}
	...
	return &goApiAbrha.AppIngressSpecRuleRoutingComponent{
...
func expandAppIngressRedirect(config []interface{}) *goApiAbrha.AppIngressSpecRuleRoutingRedirect {
	if len(config) == 0 || config[0] == nil {
		return nil
	}
	...
	return &goApiAbrha.AppIngressSpecRuleRoutingRedirect{
...
func expandAppIngressMatch(config []interface{}) *goApiAbrha.AppIngressSpecRuleMatch {
	if len(config) == 0 || config[0] == nil {
		return nil
	}
	...
	return &goApiAbrha.AppIngressSpecRuleMatch{
		Path: &goApiAbrha.AppIngressSpecRuleStringMatch{
			Prefix: path["prefix"].(string),
		},
	}
}

func flattenAppEgress(egress *goApiAbrha.AppEgressSpec) []map[string]interface{} {
	if egress != nil {
		result := make([]map[string]interface{}, 0)
...
func flattenAppIngress(ingress *goApiAbrha.AppIngressSpec) []map[string]interface{} {
	if ingress != nil {
...
func flattenAppIngressRule(rule *goApiAbrha.AppIngressSpecRule) map[string]interface{} {
	result := make(map[string]interface{}, 0)
...
func flattenAppIngressRuleComponent(component *goApiAbrha.AppIngressSpecRuleRoutingComponent) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)
...
func flattenAppIngressRuleMatch(match *goApiAbrha.AppIngressSpecRuleMatch) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)
...
func flattenAppIngressRuleRedirect(redirect *goApiAbrha.AppIngressSpecRuleRoutingRedirect) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)
